...
ll buildSegTree(int index, int leftI, int rightI)
{
    ...
    if (leftI == rightI)
    {
        segTree[index] = 1ll * timeVal[leftI];
...
ll sumQuerySegTree(int index, int leftI, int rightI,
                   int lRange, int rRange)
{
...
{
    // Approach:-
    // 1. we will construct adj list, 0 is the roor
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if (a == 1)
        {
            ...
            cin >> c;
            value[b] = c;
            updateQuerySegTree(0, 0, segTreeSize - 1, inTime[b], value[b]);
            updateQuerySegTree(0, 0, segTreeSize - 1, outTime[b], value[b]);
        }
        else
        {
            ll sum = sumQuerySegTree(0, 0, segTreeSize - 1, inTime[b], outTime[b]);
            ans.push_back(sum / 2);
        }
    }
